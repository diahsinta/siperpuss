...
if(isset($_POST['btnPinjam'])){
	...
	if(cekPinjam($koneksi, $id_anggota) && $stok > 0){
		...
		$querp=mysqli_query($koneksi,"SELECT id_pinjam FROM peminjaman WHERE tgl_pinjam='$tgl_pinjam' AND id_anggota='$id_anggota' AND tgl_pinjam_tempo='$tgl_jatuh_tempo' AND id_petugas='$id_petugas' ");
		$wd=mysqli_fetch_assoc($querp);
		$idp=$wd["id_pinjam"];
		$sql1 = mysqli_query($koneksi,"INSERT INTO detail_pinjam(id_pinjam,id_buku,tgl_kembali,status) values('$idp','$id_buku','$tgl_pinjam','dipinjam')");
